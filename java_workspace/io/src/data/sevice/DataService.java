...
public class DataService {
	...
	public void printDataByDay(String day) {
		...
		System.out.println("["+day+"] 지역사고 현황");
		System.out.println("전체 사고건수: " + datas.size());
		System.out.println("전체 사망자수: " + totalDeath);
	}
	...
	public void printStatByMonth() {
		...
		for (Data data : datas) {
			String day = data.getDay();					 //"01-01", " 01-12", "02-21", "12-11"
			String monthString = day.substring(0, 2);	// "01"      "01"     "02" 		"12"
 			int month = 0;
			if (monthString.startsWith("0")) {
				month =  Integer.parseInt(monthString.substring(1)); 	// 1  1  2
			} else {
				month = Integer.parseInt(monthString);					// 12
			}
			
			Stat savedStat = stats.get(month);	// Map에서 month를 key로 Stat객체를 조회한다.
			if (savedStat ==null) {					// m번째에 저장된 Stat객체가 없으면
				Stat stat = new Stat();				// Stat객체를 생성한다.
				stat.setDeaths(data.getDeath());	// 사망자수와 사고건수를 담아서
				stat.setRecords(1);
				
				stats.put(month, stat);			// Map에 month로 Stat룰 저장한다.
	...
	public void printStat() {
		// 최다 사고발생 시도, 최다 사고발생 월, 최다 사고 발생일, 최다 사고 발생요일, 최다 사고발생 유형
