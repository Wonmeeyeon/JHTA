...
// * FileDownloadServlet는 HttpServlet을 상속받은 서블릿 클래스다.
// * HttpServlet을 상속받은 클래스는 HTTP요청을 처리할 수 있는 서블릿이 된다.
// * FileDownloadServlet는 클라이언트의 요청을 처리할 수 있는 클래스다.
// * Tomcat은 /download URL 요청이 오면 FileDownloadServlet에 대한 요청이라고 판단한다.
// * Tomcat은 /dowmload URL 요청이 오면 요청객에와 응답객체를 생성하고,
...
// * urlPatterns는 어떤 요청 URL이 왔을 때 이 서블릿을 사용해야 하는 되는지를 지정한다.
@WebServlet(urlPatterns = "/download")
public class FileDownloadServlet  extends HttpServlet{
	
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		System.out.println("FileDownloadServlet의 service(req,resp)가 실행됨....");
	try {
		...
		System.out.println("인코딩된파일명: " + encodeFilename);
		//응답 컨텐츠의 타입을 지정한다.
		//파일의 이름을 응답메세지에 포함시킨다.
		//컨텐츠의 총 길이를 응답메세지에 포함시킨다.
