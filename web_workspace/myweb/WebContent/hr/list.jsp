...
<div class="wrapper">
	...
	<%
		...
		// 2. 클라언트가 요청한 페이지  번호 조회하기
		int pageNo = NumberUtil.stringToInt(request.getParameter("page"),1 );
		// 3. 조회할 목록의 시작번호와 끝번호를 조회한다.
		int beginRowNumber = (pageNo - 1 )*rowsPerpa +1;
		int endRowNumber = pageNo*rowsPerpa;
		System.out.println("페이지번호: " + pageNo + ", 시작번호: " + beginRowNumber + ",끝번호 :" +endRowNumber);
 		
		// 4. 화면에 표시할 목록 조회하기 
		EmployeeDao emDao = new EmployeeDao();
		//List<Employee> employees =emDao.getAllEmployees();
		List<Employee> employees =emDao.getEmployeesbyRange(beginRowNumber, endRowNumber);
 	%>
		<div>
		<p>사원 목록을 확인하세요</p>
		<table class="table">
		...
		<%	
			// 0. 한 화면당 표시할 페이지 번호 갯수
			int pagesPerBlock = 5;
		
			
			// 1. 전체 행의 갯수를 조회한다.
			int rows = emDao.getEmployeesCount();
		
			// 2. 전체 페이지수를 계산한다.
			int totalPages = (int) Math.ceil((double) rows/rowsPerpa);
			
			// 3. 전체 페이지 블록 갯수를 계산한다.
