...
<script type="text/javascript">
	function openPopup(event) {
		...
		// onreadystatechange는 XHR의 readyState 값이 변할 때 마다 발생하는 이벤트임
		xhr.onreadystatechange = function() {
			// 콜백함수가 4번 실행되기는 하지만, 실직적인 작업이 구현된 아래의 소스는
			// 하나의 XHR에 대해서 단한번만 실행된다.
			if (xhr.readyState==4 && xhr.status == 200) {
				// 서버가 보낸 응답데이터 조회<-- XHR의 responseText는 응답데이터를 보관하고있다.
				var text = xhr.responseText;
				// 응답데이터가 JSON 텍스트라면 JSON.parse(json표기법의 텍스트) 메소드를 실행해서
				// 텍스트 데이터를 자바스트립트 배열이나 객체로 변환한다.
